...
import {
  ...
  listSignalOpsIngestCredentialsV1,
...
export async function GET(request: Request) {
  ...
  try {
    const credentials = await listSignalOpsIngestCredentialsV1({ tenantId: session.tenantId });
    return json({ ok: true, requestId, credentials });
  } catch (error) {
    console.error("[SignalOps] credential listing failed", { requestId, error });
    return json({ ok: false, requestId, code: "credential_store_unavailable" }, 503);
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error("[SignalOps] credential creation failed", { requestId, error });
    return json(
      { ok: false, requestId, code: conflict ? "credential_name_conflict" : "credential_creation_failed" },
      conflict ? 409 : 503,
    );
  }
}
