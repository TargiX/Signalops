...
import { createSignalOpsClient, SignalOpsHttpError } from "./index.js";
function usage() {
...
function endpointFrom(flags, env) {
...
async function readEventInput(flags, env) {
...
function payloadEventsFromJson(value) {
...
function operatorHeaders(flags, env) {
...
async function sourceKit(flags, env) {
    return fetchJson(`${endpointFrom(flags, env)}/api/source-kit`);
}
async function sourceReport(flags, env) {
...
async function validateEvents(flags, env) {
...
function createVerificationEvent(flags, env) {
...
async function verifySource(flags, env) {
    const endpoint = endpointFrom(flags, env);
...
export async function runSignalOpsCli(argv = process.argv.slice(2), env = process.env, io = console) {
    try {
        ...
        const result = command === "source-kit"
